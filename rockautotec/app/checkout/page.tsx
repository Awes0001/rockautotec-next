...
import { CheckCircle, Lock, Truck, CreditCard, Shield, Package } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  async function handleOrder(e: React.FormEvent) {
    ...
    // Fire confirmation email (non-blocking — failure won't cancel the order)
    let oid = "";
    try {
      ...
      if (data.orderId) oid = data.orderId;
    } catch {
  ...
  if (cart.length === 0 && !placed) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-zinc-400 text-sm mb-6">Add some parts before checking out.</p>
        <Link href="/" className="text-sm font-semibold text-red-400 hover:text-red-300 transition-colors">
          ← Go Shopping
        </Link>
      </div>
    );
  ...
  if (placed) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-center px-4 py-12">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6 mb-8 text-sm text-left w-full max-w-sm space-y-4">
          {orderId && (
            <div>
              <p className="text-xs text-zinc-500 uppercase tracking-wider">Order Number</p>
              <p className="text-white font-mono font-semibold text-base mt-1">{orderId}</p>
            </div>
          )}
          <div>
            <p className="text-xs text-zinc-500 uppercase tracking-wider">Estimated Delivery</p>
            <p className="text-white font-semibold mt-1">2–5 Business Days</p>
          </div>
          <div>
            <p className="text-xs text-zinc-500 uppercase tracking-wider">Shipping To</p>
        ...
        <p className="text-xs text-zinc-600">Redirecting to home in a few seconds…</p>
        <Link href="/" className="mt-3 text-sm font-semibold text-red-400 hover:text-red-300 transition-colors">
          Return Home →
        </Link>
      </div>
    );
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  ] as [keyof FormData, string, boolean][]).map(([name, label, req]) => (
                    <div key={name}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Dallas"
                      className="w-full rounded-lg border border-zinc-700 bg-zinc-800 text-white placeholder:text-zinc-500 px-4 py-3 text-sm focus:outline-none focus:border-red-500 transition-colors"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        name="zip" required value={form.zip} onChange={handleChange}
                        placeholder="75201" maxLength={10}
                        className="w-full rounded-lg border border-zinc-700 bg-zinc-800 text-white placeholder:text-zinc-500 px-4 py-3 text-sm focus:outline-none focus:border-red-500 transition-colors"
                      />
                    </div>
                  </div>
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <input
                      name="cardName" required value={form.cardName} onChange={handleChange}
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      name="cardNumber" required value={form.cardNumber} onChange={handleChange}
                      placeholder="•••• •••• •••• ••••" maxLength={19}
                      className="w-full rounded-lg border border-zinc-700 bg-zinc-800 text-white placeholder:text-zinc-500 px-4 py-3 text-sm focus:outline-none focus:border-red-500 transition-colors font-mono"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">
                    ...
                    <input
                      name="expiry" required value={form.expiry} onChange={handleChange}
                  ...
                  <div>
                    ...
                    <input
                      name="cvv" required value={form.cvv} onChange={handleChange}
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {["Visa", "Mastercard", "Amex", "Discover", "PayPal", "Apple Pay"].map((b) => (
                    <span
                      key={b}
                      className="rounded border border-zinc-700 bg-zinc-800 px-2.5 py-1 text-xs text-zinc-500"
                    >
                      {b}
                    </span>
                  ))}
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6 sticky top-24">
                  ...
                  {cart.map((item, i) => (
                    <div key={i} className="flex items-start justify-between gap-3 text-sm">
                      <div className="flex items-start gap-2 min-w-0">
                        ...
                        <span className="text-zinc-300 leading-snug">{item.part}</span>
                      </div>
                ...
                <div className="mt-4 space-y-2">
                  {[
                    ...
                    { icon: Package,text: "Free shipping on orders $75+" },
                  ].map(({ icon: Icon, text }) => (
                ...
                <p className="text-xs text-zinc-600 mt-4 leading-relaxed">
                  ...
                  <Link href="/terms" className="text-red-400 underline">Terms</Link>{" "}
                  and{" "}
