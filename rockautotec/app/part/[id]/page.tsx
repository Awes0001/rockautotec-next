...
import {
  ...
  ChevronRight, ArrowLeft, Check, Package, Wrench, CheckCircle, Hash, Plus,
  Lock, Award, Tag,
} from "lucide-react";
...
import {
  getProductById, getRelatedProducts, getFrequentlyBoughtTogether, CATEGORY_SLUGS, CATEGORY_ICON_NAMES,
  isLowStock, lowStockCount, shippingEstimate, type Product,
} from "@/lib/catalog";
import PartIcon from "@/components/part-icon";
import ProductGallery from "@/components/product-gallery";
import BrandLogo from "@/components/brand-logo";
import { GarageBanner } from "@/components/my-garage";
...
function ProductPage() {
  ...
  // Memoized above the early `if (!product)` return so these hooks always
  // run in the same order (Phase 7 — avoids recomputing on unrelated
  // re-renders like quantity changes, without breaking React's hook rules).
  const related = useMemo(() => (product ? getRelatedProducts(product, 4) : []), [product]);
  const frequentlyBoughtTogether = useMemo(
    () => (product ? getFrequentlyBoughtTogether(product, 3) : []),
    [product]
  );
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-14">
          ...
          <div className="space-y-3">
            <div className="relative">
              ...
              />
              {savings > 0 && (
                <span className="absolute top-4 left-4 bg-red-600 text-white text-xs font-bold px-2.5 py-1 rounded-lg z-10">
                  SAVE {savings}%
                </span>
              )}
              {!product.inStock ? (
                <span className="absolute top-4 right-4 bg-zinc-600 text-zinc-200 text-xs font-semibold px-2.5 py-1 rounded-lg z-10">
                  Out of Stock
                </span>
              ) : isLowStock(product) ? (
                <span className="absolute top-4 right-4 bg-amber-600/90 text-white text-xs font-semibold px-2.5 py-1 rounded-lg z-10">
                  Only {lowStockCount(product)} left
                </span>
              ) : (
                <span className="absolute top-4 right-4 bg-emerald-600/90 text-white text-xs font-semibold px-2.5 py-1 rounded-lg z-10">
                  In Stock
                </span>
              )}
            </div>

            {/* Trust badges row (Phase 6 — 5 badges) */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {[
                { icon: Award,      label: "OEM Quality",       sub: "Meets/exceeds spec" },
                { icon: Truck,      label: "Fast Shipping",     sub: "Orders $75+ free" },
                { icon: Shield,     label: "Warranty",          sub: "12mo / 12,000mi" },
                { icon: Lock,       label: "Secure Checkout",   sub: "256-bit SSL" },
                { icon: CheckCircle,label: "Fitment Guarantee", sub: "Free return if wrong" },
            ...
            <div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">Manufacturer: {product.manufacturer}</p>
                </div>
              </div>
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <div className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-700/50 bg-emerald-950/30 px-3 py-1.5">
                  <Award className="h-3.5 w-3.5 text-emerald-400 shrink-0" />
                  <span className="text-xs text-emerald-300">OEM #:</span>
                  <span className="text-xs text-white font-mono font-bold">{product.oemNumber}</span>
                </div>
                <div className="inline-flex items-center gap-1.5 rounded-lg border border-blue-700/50 bg-blue-950/30 px-3 py-1.5">
                  <Tag className="h-3.5 w-3.5 text-blue-400 shrink-0" />
                  <span className="text-xs text-blue-300">Aftermarket #:</span>
                  <span className="text-xs text-white font-mono font-bold">{product.aftermarketNumber}</span>
                </div>
              </div>
        ...
        {frequentlyBoughtTogether.length > 0 && (
          <FrequentlyBoughtTogether product={product} extras={frequentlyBoughtTogether} />
        )}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 border-t border-zinc-800 bg-zinc-900/95 backdrop-blur px-4 py-3 flex items-center gap-3">
        ...
        <button
          onClick={handleAddToCart}
          disabled={!product.inStock}
          className="shrink-0 flex items-center justify-center gap-2 rounded-lg bg-red-600 hover:bg-red-500 active:bg-red-700 disabled:bg-zinc-700 disabled:cursor-not-allowed text-white font-bold px-5 py-2.5 text-sm transition-colors"
        >
...
const ProductTabs = memo(function ProductTabs({ product }: { product: Product }) {
  ...
  const SPECS: { label: string; value: string }[] = [
    ...
    { label: "Aftermarket Number", value: product.aftermarketNumber },
    { label: "Interchange #s",     value: product.interchangeNumbers.join(", ") },
    { label: "Brand",              value: product.brand },
    { label: "Manufacturer",       value: product.manufacturer },
    { label: "Category",           value: product.category },
    { label: "Subcategory",        value: product.partType },
...
const RelatedCard = memo(function RelatedCard({ product }: { product: Product }) {
  const { addToCart } = useCart();
  const [added, setAdded] = useState(false);
  const savings = Math.round(((product.was - product.price) / product.was) * 100);
  ...
  return (
    <Link
      ...
      className="group flex flex-col rounded-xl border border-zinc-800 bg-zinc-900 hover:border-red-500/60 hover:bg-zinc-800/80 hover:-translate-y-1 hover:shadow-lg hover:shadow-red-950/30 transition-all duration-300 overflow-hidden"
    >
      <PartIcon
        category={product.category}
        iconName={CATEGORY_ICON_NAMES[product.category]}
...
const FrequentlyBoughtTogether = memo(function FrequentlyBoughtTogether({ product, extras }: { product: Product; extras: Product[] }) {
  const { addToCart } = useCart();
  const [added, setAdded] = useState(false);
  ...
  return (
    <section className="mt-14 rounded-2xl border border-zinc-800 bg-zinc-900 p-5 sm:p-6">
      <h2 className="text-xl sm:text-2xl font-bold text-white mb-5">Frequently Bought Together</h2>

      <div className="flex items-stretch gap-2 mb-5 overflow-x-auto pb-1">
        {bundle.map((p, i) => (
          <div key={p.id} className="flex items-center gap-2 sm:gap-3 shrink-0">
            <Link
              href={`/part/${p.id}`}
              className="flex flex-col items-center gap-2 w-28 sm:w-32 text-center group"
            >
              <PartIcon
                category={p.category}
                iconName={CATEGORY_ICON_NAMES[p.category]}
                className="h-20 w-20 sm:h-24 sm:w-24 rounded-xl"
                iconClassName="h-8 w-8 sm:h-9 sm:w-9 group-hover:scale-110 transition-transform duration-200"
              />
              ...
              {i === 0 && <span className="text-[10px] font-bold text-red-400 uppercase tracking-wider">This Item</span>}
            </Link>
            {i < bundle.length - 1 && <Plus className="h-4 w-4 text-zinc-600 shrink-0" />}
          </div>
        ))}
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl bg-zinc-950 border border-zinc-800 px-4 py-3.5">
        <p className="text-sm text-zinc-300">
          Buy all {bundle.length} items: <span className="text-lg font-bold text-white">${bundleTotal.toFixed(2)}</span>
        </p>
        <button
          onClick={addBundleToCart}
          className={`inline-flex items-center justify-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold transition-colors shrink-0 ${
            added ? "bg-emerald-600 text-white" : "bg-red-600 hover:bg-red-500 active:bg-red-700 text-white"
          }`}
        >
          {added ? <Check className="h-4 w-4" /> : <ShoppingCart className="h-4 w-4" />}
